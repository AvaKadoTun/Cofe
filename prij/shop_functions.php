...
{
    if (is_wp_error(wp_signon(array(
...
function ajaxs_wc_register($jx)
{
    ...
    if (is_wp_error($result)) {
        $jx->error($result->get_error_message());
    } else {
        $jx->success();
    }
}
...
{
    WC()->cart->add_to_cart($jx->data['product_id'], $jx->data['qty'], $jx->data['variation_id'], $jx->data['variation_attributes'], $jx->data['item_data']);
